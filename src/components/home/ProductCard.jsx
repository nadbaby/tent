...
const resolveImageUrl = (imagePath) => {
  ...
  if (httpMatch) {
    resolvedUrl = httpMatch[0];
  } else if (imagePath.startsWith('http://') || imagePath.startsWith('https://')) {
    resolvedUrl = imagePath;
  } else {
    const cleanPath = imagePath.startsWith('/') ? imagePath : `/${imagePath}`;
    resolvedUrl = apiUrl(cleanPath);
  }

  if (resolvedUrl.includes('res.cloudinary.com') && !resolvedUrl.includes('q_auto')) {
    resolvedUrl = resolvedUrl.replace('/upload/', '/upload/w_500,c_limit,q_auto,f_auto/');
  }
...
const ProductCard = ({ product, isAdmin, onEdit, onDelete, searchTerm, onCardClick, displayName, isSelected, onSelectToggle, onAddToCart }) => {
  ...
  return (
    <div className={`product-card ${isSelected ? 'admin-selected' : ''} ${isShaking ? 'shake-animation' : ''}`} onClick={handleCardClick}>
      ...
      <div className="product-info">
        ...
        <h3 className="product-name">{highlightText(displayName || product.name || product.subcategory || 'Product', searchTerm)}</h3>
