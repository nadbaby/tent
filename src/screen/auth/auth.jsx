...
const Auth = () => {
  ...
  async function syncUserWithBackend(idToken, extraData = {}) {
    try {
      ...
      const fallbackUser = {
        name: extraData.name || extraData.email?.split('@')[0] || 'User',
  ...
  const handlePasswordSubmit = async (e) => {
    ...
    try {
      ...
      // If the user entered a username instead of an email, resolve it
      if (!loginEmail.includes('@')) {
        const res = await fetch(apiUrl('/api/auth/resolve-username'), {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
        ...
        if (res.ok) {
          ...
          setErrorMsg("Username not found. Please check your username or try logging in with your email.");
          setIsLoading(false);
          return;
        }
      }
  ...
  const handleSignupSubmit = async (e) => {
    ...
    if (!trimmedPhone) {
      // If phone is not provided, proceed directly with account creation
      try {
        const syncedUser = await executeRegistration(null);
        let msg = 'Account created successfully! Redirecting...';
        if (syncedUser?.gstNumber) {
          const couponRes = await getEligibleGstCoupon(syncedUser.gstNumber);
          if (couponRes && couponRes.eligible) {
    ...
    // If phone is provided, send OTP first and ask user for OTP code
    try {
      const res = await fetch(apiUrl('/api/auth/send-otp'), {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      if (res.ok) {
        ...
        setResendTimer(30);
        let successMsgText = 'OTP sent to your phone! Please enter the code to complete signup.';
        if (data.devOtp) {
          successMsgText += ` (Dev OTP: ${data.devOtp})`;
        }
      ...
      setErrorMsg(friendlyError(err.message || 'Failed to send OTP'));
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleVerifyEmailSignupOtp = async (e) => {
    e.preventDefault();
    if (isLoading) return;
    const cleanOtp = String(emailSignupOtp || '').trim();
    if (!cleanOtp) return setErrorMsg('Please enter the 6-digit OTP');
    ...
    try {
      ...
      const res = await fetch(apiUrl('/api/auth/verify-otp-only'), {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ phone: phone.trim(), otp: cleanOtp }),
      });
      const data = await parseJsonResponse(res);
  ...
  const friendlyError = (code) => {
    if (typeof code === 'string' && (code.includes("Unexpected end of JSON input") || code.includes("Failed to execute 'json'"))) {
      return 'Server response error. Please check your backend connection and try again.';
    }
    switch (code) {
      ...
      default:
        if (typeof code === 'string' && code.trim() && !code.startsWith('auth/')) {
          return code;
        }
