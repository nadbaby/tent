...
import { Search, Edit2, Save, X, CreditCard, Users, Shield, Package, RefreshCw, Filter, Download, Calendar, MapPin, Eye, FileText, ChevronRight, AlertCircle, Clock, Truck, CheckCircle, Info, SortAsc, SortDesc, Megaphone, DollarSign, LifeBuoy } from 'lucide-react';
        ...
        <div className="orders-dashboard-list">
          {sortedOrders.length > 0 ? (
            sortedOrders.map(order => (
              <div key={order.orderId} className={`processing-card ${order.paymentDetails?.status === 'FAILED' ? 'failed' : ''}`}>
                <div className="pc-header">
                  <div className="pc-order-id">
                    <h3>Order #{order.orderId}</h3>
                    <span className="pc-date"><Clock size={14} /> {formatDate(order.createdAt)}</span>
                  </div>
                  <div className="pc-status-group">
                    <div className={`pc-pay-status ${order.paymentDetails?.status?.toLowerCase() || 'pending'}`}>
                      {order.paymentDetails?.status || 'PENDING'}
                      {order.paymentDetails?.transactionId && <span style={{ fontSize: '0.7rem', display: 'block', textTransform: 'none', marginTop: '2px' }}>TXN: {order.paymentDetails.transactionId}</span>}
                    </div>
                    ...
                    {editingOrderId === order.orderId ? (
                      <div className="inline-edit">
                        ...
                        <div className="edit-actions" style={{ display: 'flex', gap: '4px', marginLeft: '8px' }}>
                          <button className="btn-action edit" onClick={() => handleUpdateStatus(order.orderId)} style={{ padding: '4px', background: '#e2e8f0', border: 'none', borderRadius: '4px' }}><Save size={16} /></button>
                          <button className="btn-action view" onClick={() => setEditingOrderId(null)} style={{ padding: '4px', background: '#fee2e2', color: '#991b1b', border: 'none', borderRadius: '4px' }}><X size={16} /></button>
                        </div>
                      </div>
                    ) : (
                      <div className={`pc-order-status ${order.status.toLowerCase().replace(' ', '-')}`}>
                        {order.status}
                      </div>
                    )}

                    {!editingOrderId || editingOrderId !== order.orderId ? (
                      <button className="btn-action edit" onClick={() => { setEditingOrderId(order.orderId); setNewStatus(order.status); }} title="Quick Status Edit" style={{ padding: '6px', background: '#f1f5f9', border: 'none', borderRadius: '50px', cursor: 'pointer' }}>
                        <Edit2 size={16} color="#475569" />
                      </button>
                    ) : null}

                    <button className="btn-action view" onClick={() => { setSelectedOrder(order); setActiveModalTab('overview'); }} title="View Full Details" style={{ padding: '6px', background: '#eff6ff', border: 'none', borderRadius: '50px', cursor: 'pointer' }}>
                ...
                <div className="pc-body">
                  <div className="pc-customer-section">
                    <h4>Customer & Delivery</h4>
                    <p style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                      <strong style={{ fontSize: '1.05rem', color: '#0f172a' }}>{order.shippingAddress?.fullName || 'Guest'}</strong>
                      {order.shippingAddress?.gstNumber && <span className="pc-variant" style={{ background: '#e0e7ff', color: '#3730a3' }}>GST: {order.shippingAddress.gstNumber}</span>}
                    </p>
                    <p style={{ marginTop: '8px' }}>{order.shippingAddress?.email}</p>
                  ...
                  <div className="pc-products-section">
                    <h4>Products ({order.items?.length || 0})</h4>
                    <div className="pc-product-list">
                      {order.items?.map((item, i) => {
                        const dbProd = productsList.find(p => String(p.id) === String(item.id));
                        return (
                          <div key={i} className="pc-product-item">
                            <div className="pc-prod-img">
                              {dbProd?.imagePublicId
                                ? <img src={`https://res.cloudinary.com/dwyxshzwb/image/upload/c_fit,w_100,h_100/${dbProd.imagePublicId}`} alt={item.name} />
                                : <div className="img-placeholder"><Package size={20} /></div>
                              }
                            </div>
                            <div className="pc-prod-details">
                              <h5>{item.name}</h5>
                              <div className="pc-prod-meta">
                                <span className="category">{dbProd?.category || 'General'} {dbProd?.subcategory ? ` > ${dbProd.subcategory}` : ''}</span>
                                <span className="brand">Brand: {dbProd?.brand || 'N/A'}</span>
                                <span className="sku">SKU: {dbProd?.sku || item.id || 'N/A'}</span>
                              </div>
                              {item.size && <span className="pc-variant">Variant/Size/Voltage: {item.size}</span>}
                            </div>
                            <div className="pc-prod-price">
                              <span className="qty">{item.quantity} × ₹{item.price?.toFixed(2)}</span>
                    ...
                    <div className="pc-footer-total">
                      {order.discountAmount > 0 && <span className="pc-discount">Discount: -₹{order.discountAmount.toFixed(2)}</span>}
                      {order.shippingCharge > 0 && <span style={{ color: '#64748b' }}>Shipping: +₹{order.shippingCharge.toFixed(2)}</span>}
            ...
            <div className="empty-state-premium animate-in">
              ...
              <p>Looks like things are quiet. Orders placed today will automatically appear here with full processing details.</p>
            </div>
          )}
        </div>
      </div>
          ...
          <div className="ops-drawer" onClick={e => e.stopPropagation()}>
            <div className="drawer-header">
              ...
              <div className="header-actions">
                <button className="btn-print" onClick={() => printInvoice(selectedOrder)}><FileText size={18} /> Invoice</button>
            ...
            <div className="drawer-footer" style={{ display: 'flex', gap: '10px', justifyContent: 'flex-end' }}>
              <button className="btn-outline" onClick={() => printInvoice(selectedOrder)} style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
